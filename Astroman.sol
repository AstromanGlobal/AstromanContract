...
contract Astroman is Context, IERC20, Ownable {
    ...
    uint256 private _numTokensSellToAddToLiquidity = 500 * 10**3 * 10**9;
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferBurn) public view returns(uint256) {
    ...
    function _getValues(uint256 tAmount) private view returns (uint256, uint256, uint256, uint256, uint256, uint256) {
        ...
        (uint256 rAmount, uint256 rTransferAmount, uint256 rBurn) = _getRValues(tAmount, tBurn, tLiquidity, _getRate());
        return (rAmount, rTransferAmount, rBurn, tTransferAmount, tBurn, tLiquidity);
    }
    ...
    function _getRValues(uint256 tAmount, uint256 tBurn, uint256 tLiquidity, uint256 currentRate) private pure returns (uint256, uint256, uint256) {
    ...
    function _transferStandard(address sender, address recipient, uint256 tAmount) private {
        (uint256 rAmount, uint256 rTransferAmount, uint256 rBurn, uint256 tTransferAmount, uint256 tBurn, uint256 tLiquidity) = _getValues(tAmount);
        _rOwned[sender] = _rOwned[sender].sub(rAmount);
